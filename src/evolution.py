...
class HighDimEvolution:
    def _compute_advantages(self, fitnesses):
        t_fitnesses = torch.tensor(fitnesses, dtype=torch.float32)
        if t_fitnesses.numel() < 2:
            return torch.zeros_like(t_fitnesses)

        std = t_fitnesses.std()
        if std < 1e-8:
            return torch.zeros_like(t_fitnesses)

        return (t_fitnesses - t_fitnesses.mean()) / std
