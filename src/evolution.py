...
class HighDimEvolution:
    ...
    def _apply_update(self, population: List[nn.Module], update: Any) -> None:
        ...
        for model in population:
            if isinstance(update, torch.Tensor):
                ...
                for param in model.parameters():
                    numel = param.numel()
                    param_update = update[offset:offset+numel].view(param.size())
                    with torch.no_grad():
                        param.data.add_(param_update)
                    offset += numel
            elif isinstance(update, (list, tuple)):
                for param, param_update in zip(model.parameters(), update):
                    with torch.no_grad():
                        param.data.add_(param_update)
    ...
    def trpo_step(self, population: List[nn.Module], fitnesses: List[float]):
        ...
        return self._apply_update(population, natural_grad * step_size)
