    ...
    def forward(self, research_state: ResearchState) -> ResearchAction:
        ...
        if isinstance(self.successful_experiments, list):
            if not self.successful_experiments:
                is_empty = True
        elif isinstance(self.successful_experiments, torch.Tensor):
            if self.successful_experiments.nelement() == 0:
                is_empty = True
        ...
        # Vectorized implementation using torch.cdist
        query = research_state.embedding
        if not isinstance(query, torch.Tensor):
            query = torch.tensor(query, dtype=torch.float32)
        if query.dim() == 1:
            query = query.unsqueeze(0)  # (1, D)

        if isinstance(self.successful_experiments, torch.Tensor):
            exps = self.successful_experiments
        elif len(self.successful_experiments) > 0 and isinstance(self.successful_experiments[0], torch.Tensor):
            exps = torch.stack(self.successful_experiments)
        else:
            exps = torch.tensor(self.successful_experiments, dtype=torch.float32)

        # Ensure query is on the same device as exps
        if query.device != exps.device:
            query = query.to(exps.device)

        if exps.dim() == 2:
            # query: (1, D), exps: (N, D) -> dists: (1, N)
            dists = torch.cdist(query, exps)
